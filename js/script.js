...
// ---------- Wandernder Lichtimpuls auf einzelnen Woertern ----------
...
(function () {
  ...
  document.querySelectorAll("[data-leuchtwort]").forEach((wort) => {
...
// ---------- Bewertungsstrecke ----------
// Fuehrt Schritt fuer Schritt durch das Formular. Alle Schritte bleiben
// im Dokument stehen — nur der aktive ist sichtbar. So gehen beim
// Absenden saemtliche Angaben mit, ganz ohne Neuladen.
(function () {
  const formular = document.querySelector("form[data-bewertung]");
  if (!formular) return;

  const strecke = formular.closest(".bw-strecke");
  const schritte = [...formular.querySelectorAll(".bw-schritt")];
  const balken = strecke.querySelector("[data-fortschritt]");
  const nummer = strecke.querySelector("[data-schritt-nr]");
  const zurueck = formular.querySelector("[data-zurueck]");
  const weiter = formular.querySelector("[data-weiter]");
  const hinweis = formular.querySelector("[data-bw-hinweis]");
  if (!schritte.length) return;

  let aktuell = 0;

  const objektart = () => {
    const gewaehlt = formular.querySelector('input[name="typ"]:checked');
    return gewaehlt ? gewaehlt.value : "";
  };

  // Manche Angaben ergeben nur fuer bestimmte Objektarten Sinn — ein
  // Stockwerk beim Grundstueck etwa nicht. Ausgeblendete Felder werden
  // geleert, damit sie nicht doch in der Mail landen.
  function passendeFelder() {
    const art = objektart();
    formular.querySelectorAll("[data-nur]").forEach((el) => {
      const passt = el.dataset.nur.split(" ").includes(art);
      el.hidden = !passt;
      if (!passt) {
  ...
  // Ein ganzer Schritt kann entfallen (Zustand & Ausstattung beim
  // Grundstueck). Deshalb wird die Liste der gueltigen Schritte jedes
  // Mal neu bestimmt statt fest durchgezaehlt.
  const gueltig = () =>
    schritte.filter((s) => {
      const nur = s.dataset.nur;
      return !nur || !objektart() || nur.split(" ").includes(objektart());
    });

  function zeigen(index) {
    const liste = gueltig();
    aktuell = Math.max(0, Math.min(index, liste.length - 1));

    schritte.forEach((s) => s.classList.remove("ist-aktiv"));
    liste[aktuell].classList.add("ist-aktiv");

    const anteil = ((aktuell + 1) / liste.length) * 100;
    if (balken) balken.style.width = anteil + "%";
    if (nummer) {
      nummer.textContent = aktuell + 1;
      nummer.closest(".bw-fortschritt-text").lastChild.textContent =
        " von " + liste.length;
    }

    if (zurueck) zurueck.hidden = aktuell === 0;
    // Im letzten Schritt sendet der Knopf im Schritt selbst ab.
    if (weiter) weiter.hidden = aktuell === liste.length - 1;
    if (hinweis) hinweis.hidden = true;
  }

  function geprueft() {
    const schritt = gueltig()[aktuell];

    // Schritt 1: ohne Objektart geht es nicht weiter, sonst wissen wir
    // nicht, welche Felder ueberhaupt gefragt sind.
    if (schritt.dataset.schritt === "1" && !objektart()) {
      return "Bitte wähl aus, was du bewerten möchtest.";
    }
    if (schritt.dataset.schritt === "2") {
      const plz = formular.plz.value.trim();
      const ort = formular.ort.value.trim();
      if (!plz && !ort) return "Bitte gib zumindest PLZ oder Ort an.";
    }
    return "";
  }
  ...
  weiter?.addEventListener("click", () => {
    const fehler = geprueft();
    if (fehler) { melden(fehler); return; }
    melden("");
    zeigen(aktuell + 1);
    strecke.scrollIntoView({ behavior: "smooth", block: "start" });
  });
  ...
  // Die Objektart entscheidet ueber Felder und Schrittzahl.
  formular.querySelectorAll('input[name="typ"]').forEach((f) =>
    f.addEventListener("change", () => { passendeFelder(); zeigen(aktuell); })
  );

  // Der eigentliche Versand laeuft ueber den allgemeinen Formular-
  // Handler weiter oben. Hier wird nur vorher geprueft, ob das
  // Noetigste ausgefuellt ist.
  formular.addEventListener("submit", (e) => {
    const email = formular.email.value.trim();
    const vorname = formular.vorname.value.trim();
    if (!vorname || !email || !email.includes("@")) {
      e.preventDefault();
      e.stopImmediatePropagation();
      melden("Bitte gib Vorname und E-Mail an, damit wir dir antworten können.");
      return;
    }
    melden("");
  }, true);
